...
char* str_slicing(char* A, int start, int end){
    char* ans = (char*)malloc(sizeof(char)*(end-start+1));
...
void delete_idx_array(int* array, int idx, int* array_len){
    for (int i = idx; i < *array_len-1; i++){
        array[i] = array[i+1];
    }
    *array_len -= 1;
}
